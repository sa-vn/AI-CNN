...
class ImprovedCNN(nn.Module):
    def __init__(self):
        ...
        self.fc1 = nn.Linear(1024 * 4 * 4, 1024)  # Adjusted for new layer and increased units
        self.fc2 = nn.Linear(1024, 512)  # Additional hidden layer
        self.fc3 = nn.Linear(512, 256)  # Additional hidden layer
        self.fc4 = nn.Linear(256, 128)
        self.fc5 = nn.Linear(128, 5)  # 5 classes
    ...
    def forward(self, x):
        ...
        x = self.pool(torch.relu(self.batch_norm6(self.conv6(x))))  # Additional layer forward
...
# Adjusted training function to track and plot metrics and confusion matrices
def train_model(model, train_loader, val_loader, criterion, optimizer, scheduler, num_epochs=25):
    ...
    for epoch in range(num_epochs):
        ...
        if (epoch + 1) % 5 == 0 or epoch + 1 == num_epochs:
            ...
            plot_confusion_matrix(conf_matrix, f'Validation Confusion Matrix Epoch {epoch+1}')
...
def evaluate_model(model, test_loader, title):
    ...
    print(f'{title} Accuracy: {accuracy:.2f}, {title} Precision: {precision:.4f}, {title} Recall: {recall:.4f}, '
          f'{title} F1 Score: {f1:.4f}')
    print(f'Confusion Matrix:\n{conf_matrix}')
    plot_confusion_matrix(conf_matrix, f'{title} Confusion Matrix')
